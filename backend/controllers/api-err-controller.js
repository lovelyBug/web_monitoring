...
class HttpErrorInfoController {
  ...
  static async getApiErrInfoByHour(ctx) {
    try {
      ...
      const sql = `SELECT statusText, httpUrl, status, loadTime, COUNT(httpUrl) as num, happenDate, COUNT(DISTINCT userId) as userNum FROM ${tableName} WHERE happenDate BETWEEN '${startTime}' and '${endTime}' and statusResult = '请求返回' GROUP BY httpUrl`
  ...
  static async getApiErrInfoByType(ctx) {
    try {
  ...
  static async getApiErrInfoByID(ctx) {
    try {
