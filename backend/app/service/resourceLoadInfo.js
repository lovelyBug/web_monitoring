...
class ResourceLoadInfo extends Service {
  constructor(ctx) {
    super(ctx);
    // 将用户的模型赋值在全局
    this.model = ctx.model.ResourceLoadInfo;
  }

  async saveResourceLoadInfo(logInfo) {
    try {
  ...
  async getErrorDetail(errId) {
    try {
      const errDetail = await this.model.findAll({ where: { id: errId } });
      return errDetail
    } catch (e) {
      console.log(e);
    }
